...
# 3.c) Enforce non-null constraints by quarantining violating records.
# Instead of failing the job, we separate records with nulls on required
# columns and write them to a _rejected/ prefix for human review.
null_counts = (
    raw_df.select([
...
if null_violations:
    print(
        f"[ETL] WARNING: Non-null constraint violated on required columns: "
        f"{null_violations} (total={sum(null_violations.values())})"
    )
    
    # Build condition to identify records with ANY null in required columns
    null_condition = None
    for col_name in REQUIRED_NON_NULL_COLUMNS:
        col_is_null = F.col(col_name).isNull()
        null_condition = col_is_null if null_condition is None else (null_condition | col_is_null)
    
    # Split into rejected and valid records
    rejected_df = raw_df.filter(null_condition)
    raw_df = raw_df.filter(~null_condition)
    
    rejected_count = rejected_df.count()
    valid_count = raw_df.count()
    
    # Write rejected records to quarantine
    rejected_path = f"s3://{args['silver_bucket']}/_rejected/transactions/reason=null_required_fields/"
    rejected_df.write.mode("append").json(rejected_path)
    
    print(f"[ETL] quarantined {rejected_count} records with null required fields to {rejected_path}")
    print(f"[ETL] continuing with {valid_count} valid records")
    
    if valid_count == 0:
        raise RuntimeError("All records violated non-null constraints — no valid data to process")
